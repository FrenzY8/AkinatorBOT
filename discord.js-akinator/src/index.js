...
module.exports = async function (input, options = {}) {
    ...
    try {
        ...
        let startingEmbed = {
            title: `${await translate("Just a seconds... and this GAME will be STARTED", options.language)}`,
            description: `**${await translate("Think about some CHARACTER and the Akinator will trying to Answer it!", options.language)}**`,
        ...
        let noResEmbed = {
            title: `**${await translate(`This game was ended because of one request`, options.language)}**`,
            description: `**${await translate(`${inputData.author.username}, This game was succesfully force-ended`, options.language)}**`,
            color: options.embedColor,
            author: { name: usertag, icon_url: avatar }
        }

        let akiEmbed = {
            title: `**${await translate(`Still WAITING for the first ANSWER...\nQuestion Number: ${aki.currentStep + 1}`, options.language)}**`,
            description: `**${translations.progress}: 0%\n${await translate(aki.question, options.language)}**`,
            color: options.embedColor,
            fields: [],
            author: { name: usertag, icon_url: avatar },
            footer: { text: `${await translate(`Well if you feel like this game boring, you can force-stop it by write 's'`, options.language)}` }
        }
        ...
        while (notFinished) {
            ...
            if (((aki.progress >= 95 && (stepsSinceLastGuess >= 10 || hasGuessed == false)) || aki.currentStep >= 78) && (!attemptingGuess.has(inputData.guild.id))) {
                ...
                let guessEmbed = {
                    title: `**${await translate("HMMM! Im sure your character is...", options.language)}**`,
                ...
                console.log(`Guessing: ${aki.answers[0].name} from ${message.author.tag}`)
                    ...
                    .then(async response => {
                        ...
                        if (guessAnswer == "y" || guessAnswer == translations.yes.toLowerCase()) {
                            let finishedGameCorrect = {
                                title: `**${await translate("NICE Game!", options.language)}**`,
                                description: `**${await translate(`${inputData.author.username} Well ill try to Guess it once again!`, options.language)}**`,
                                color: options.embedColor,
                                author: { name: usertag, icon_url: avatar },
                                fields: [
                            ...
                            if (aki.currentStep >= 78) {
                                let finishedGameDefeated = {
                                    title: `**${await translate("Well its a good game!", options.language)}**`,
                                    description: `**${await translate(`You just defeated me :( Congrats!`, options.language)}**`,
                                    color: options.embedColor,
                                    author: { name: usertag, icon_url: avatar }
                                }
            ...
            if (aki.currentStep !== 0) {
                let updatedAkiEmbed = {
                    title: `**${await translate(`Trying to thinking... Question Number: ${aki.currentStep + 1}`, options.language)}**`,
                    description: `**${await translate(`**__Chance__ to get the __character__ name: ${Math.round(aki.progress)}%\n${await translate(aki.question, options.language)}**`, options.language)}**`,
                    color: options.embedColor,
                    fields: [],
                    author: { name: usertag, icon_url: avatar },
                    footer: { text: `${await translate(`Well if you feel like this game boring, you can force-stop it by write 's'`, options.language)}` }
            ...
            await awaitInput(options.useButtons, inputData, akiMessage, false, translations, options.language)
                .then(async response => {
                    ...
                    const mikir = `${await translate(`Thinking about your character is... Just wait a second...`, options.language)}`
                    const anjayLah = `${await translate(`Chance to get the character name: ${Math.round(aki.progress)}%`, options.language)}` //Chance to get the** __character__ **name: ${Math.round(aki.progress)}%
                    const goodNice = `${await translate(`Question Number: ${aki.currentStep + 1}`, options.language)}`
                    let thinkingEmbed = {
                        title: `${goodNice}`,
                        description: `**${anjayLah}**`,
                        color: options.embedColor,
                        fields: [],
                        author: { name: usertag, icon_url: avatar },
                        footer: { text: mikir }
                    ...
                    if (answer == "b" || answer == translations.back.toLowerCase()) {
                        ...
                        let stopEmbed = {
                            title: `**${await translate(`This Game is Force-Ended`, options.language)}**`,
                            description: `**${await translate(`This game was ended because you just request it!!`, options.language)}**`,
                            color: options.embedColor,
                            author: { name: usertag, icon_url: avatar }
                        }
